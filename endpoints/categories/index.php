...
{
    $sql = "SELECT * FROM categories_defined";
    $result  = $conn -> query($sql);
    if($result->num_rows==0){
        ...
        $outp = array();
        $outpC = array('code' => 200, 'state' => 'success', 'message' => 'Categories are available');
...
{
    $arr = array('code' => 200, 'state' => 'success', 'message' => 'Shows the available endpoints.','action'=>'create/view/delete','name'=>'Place name here','description'=>'Place description here', 'example1'=>'/categories/?action=view');
    header('Content-type: application/json');
    echo json_encode($arr);
}